...
Этот документ описывает процесс размещения статического сайта в бакете `ai-craft-prod` на Yandex Object Storage.
...
Если бакет `ai-craft-prod` ещё не создан, создайте его через консоль Yandex Cloud или выполните:

```bash
aws --endpoint-url=https://storage.yandexcloud.net s3 mb s3://ai-craft-prod
```
...
```bash
aws --endpoint-url=https://storage.yandexcloud.net s3 sync ./ s3://ai-craft-prod/ \
  --exclude ".DS_Store" \
  --exclude "__pycache__/*" \
  --exclude "*.pyc" \
  --exclude ".git/*" \
...
1. Откройте бакет `ai-craft-prod`
...
aws --endpoint-url=https://storage.yandexcloud.net s3 cp \
  s3://ai-craft-prod/index.html s3://ai-craft-prod/index.html \
  --content-type "text/html; charset=utf-8" \
  --metadata-directive REPLACE

# CSS
aws --endpoint-url=https://storage.yandexcloud.net s3 cp \
  s3://ai-craft-prod/styles.css s3://ai-craft-prod/styles.css \
  --content-type "text/css; charset=utf-8" \
  --metadata-directive REPLACE

# JavaScript
aws --endpoint-url=https://storage.yandexcloud.net s3 cp \
  s3://ai-craft-prod/src/main.js s3://ai-craft-prod/src/main.js \
...
**Основной URL:**
```
https://ai-craft-prod.website.yandexcloud.net
```

**Альтернативный URL:**
```
https://ai-craft-prod.storage.yandexcloud.net
```

## Настройка собственного домена (опционально)

1. В консоли Yandex Cloud откройте бакет `ai-craft-prod`
2. Перейдите в раздел "Веб-сайт"
