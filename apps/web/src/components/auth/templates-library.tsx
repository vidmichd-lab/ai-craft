...
import { Banner, Button, Input, SectionHeader } from '@ai-craft/ui';
            ...
            <div className={styles.stack}>
              <div className={styles.heroStats}>
                <div className={styles.heroStat}>
                  <div className={styles.heroStatLabel}>Шаблоны</div>
                  <div className={styles.heroStatValue}>{templates.length}</div>
                  <div className={styles.heroStatHint}>Всего в библиотеке команды</div>
                </div>
                <div className={styles.heroStat}>
                  <div className={styles.heroStatLabel}>В выдаче</div>
                  <div className={styles.heroStatValue}>{filteredTemplates.length}</div>
                  <div className={styles.heroStatHint}>После текущего поиска</div>
                </div>
                <div className={styles.heroStat}>
                  <div className={styles.heroStatLabel}>Последнее обновление</div>
                  <div className={styles.heroStatValue}>
                    {latestTemplate ? new Date(latestTemplate.createdAt).toLocaleDateString('ru-RU') : '—'}
                  </div>
                  <div className={styles.heroStatHint}>
                    {latestTemplate ? latestTemplate.authorName || 'Неизвестный автор' : 'Пока без сохранений'}
                  </div>
                </div>
                <div className={styles.heroStat}>
                  <div className={styles.heroStatLabel}>Источник</div>
                  <div className={styles.heroStatValue}>Editor</div>
                  <div className={styles.heroStatHint}>Новые шаблоны сохраняются прямо из рабочей сцены</div>
                </div>
              </div>
              {filteredTemplates.length ? (
                ...
                <div className={styles.emptyRich}>
                  {templates.length
                    ...
                    : 'Шаблонов пока нет. Сохрани первую удачную сцену из редактора, и она сразу появится здесь для всей команды.'}
                </div>
              )}
            </div>
            <aside className={`${styles.sidebarColumn} ${styles.stickyPanel}`}>
              <div className={styles.sidebarCard}>
                <div className={styles.sidebarLabel}>Фокус</div>
                <div className={styles.sidebarTitle}>
                  {latestTemplate ? latestTemplate.name : 'Пока без шаблонов'}
                </div>
              ...
              <div className={styles.sidebarCard}>
                <div className={styles.sidebarLabel}>Как это работает</div>
