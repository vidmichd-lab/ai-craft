...
const LEGACY_ASSET_BASE = process.env.NEXT_PUBLIC_LEGACY_ASSET_BASE_URL || 'https://ai-craft.website.yandexcloud.net';

type Props = {
  state: EditorSnapshot;
};

export function LegacyPreview({ state }: Props) {
  ...
  const downloadAll = () => {
    DEFAULT_PREVIEW_SURFACES.forEach((surface: PreviewSurface) => {
      const safeBrand = String(state.brandName || 'layout')
        .trim()
        .toLowerCase()
        .replace(/\s+/g, '-')
        .replace(/[^a-z0-9_-]+/gi, '');
          ...
          <section className={styles.canvasCard} key={surface.key}>
            ...
            <div className={styles.actionsRow}>
              <button
                ...
                type="button"
                onClick={() => downloadCanvas(surface.key, `${state.brandName || 'layout'}-${surface.width}x${surface.height}.png`)}
              >
                Скачать PNG
              </button>
            </div>
